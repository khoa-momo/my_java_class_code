...
public class BookManager {
    ...
    public void visitNoLended(Node p) {
        if(p!=null) 
...
////Functions     
    ...
    public int checkDupBcode(String xbCode){         
        Node p = head;
        while(p != null){
    ...
    public Node searchByBcode(String xbCode){         
        Node p = head;
        while(p != null){
            if(p.info.bcode.equals(xbCode)) 
                return(p);
    ...
    public void deleteByBcode(Node q) {
    ...
    public void insertAfter(Node q, Book x){
    ...
    //Check Bcode for Lending
    public int checkBcode(String xbCode){         
        Node p = head;
        while(p != null){
            if(p.info.bcode.equals(xbCode)) 
    ...
    public int checkLendQuantity(){         
        Node p = head;
        while(p != null){
            if(p.info.lended == p.info.quantity) 
                return 1; //lended = quantity 
            if(p.info.lended < p.info.quantity) 
                return 2; //lended < quantity 
            p = p.next;
        } 
        return -1;
    }            
